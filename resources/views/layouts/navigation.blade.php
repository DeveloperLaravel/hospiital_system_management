<nav class="flex-1 p-4 space-y-6 text-sm overflow-y-auto" x-data="navigation()" @click="handleLinkClick($event)">
    ...
    <div>
        <a href="{{ route('dashboard') }}" class="sidebar-item {{ request()->routeIs('dashboard') ? 'sidebar-active' : '' }}">
            <x-icon name="home" />
            <span :class="collapsed ? 'hidden' : ''">لوحة التحكم</span>
        </a>
    </div>
    ...
    <div>
        <p class="sidebar-section" :class="collapsed ? 'hidden' : ''">إدارة النظام</p>

        <div class="space-y-1">
            @can('manage user')
            <a href="{{ route('users.index') }}" class="sidebar-item {{ request()->routeIs('users.*') ? 'sidebar-active' : '' }}">
                <x-icon name="users" />
                <span :class="collapsed ? 'hidden' : ''">المستخدمون</span>
            </a>
            @endcan
            ...
            <a href="{{ route('roles.index') }}" class="sidebar-item {{ request()->routeIs('roles.*') ? 'sidebar-active' : '' }}">
                <x-icon name="shield" />
                <span :class="collapsed ? 'hidden' : ''">الأدوار</span>
            </a>
            @endcan
            ...
            <a href="{{ route('permissions.index') }}" class="sidebar-item {{ request()->routeIs('permissions.*') ? 'sidebar-active' : '' }}">
                <x-icon name="key" />
                <span :class="collapsed ? 'hidden' : ''">الصلاحيات</span>
            </a>
            @endcan
        </div>
    </div>
    ...
    {{-- ===================== الإدارة المالية ===================== --}}
    <div>
        <p class="sidebar-section" :class="collapsed ? 'hidden' : ''">الإدارة المالية</p>

        <div class="space-y-1">
            <a href="{{ route('invoices.index') }}" class="sidebar-item {{ request()->routeIs('invoices.*') ? 'sidebar-active' : '' }}">
                <x-icon name="cash" />
                <span :class="collapsed ? 'hidden' : ''">الفواتير</span>
            </a>
        </div>
    </div>
