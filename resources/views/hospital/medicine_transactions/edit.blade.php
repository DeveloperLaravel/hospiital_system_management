...
<div class="p-6 bg-gray-50 min-h-screen" dir="rtl">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">تعديل حركة دواء</h1>
                <p class="text-gray-600 mt-1">تعديل بيانات حركة الدواء</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('medicine-transactions.update', $transaction) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="medication_id" id="medication_id"
                            ...
                            @foreach($medications as $m)
                                <option value="{{ $m->id }}" {{ $transaction->medication_id == $m->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            ...
                            <option value="in" {{ $transaction->type === 'in' ? 'selected' : '' }}>إدخال (إضافة للمخزون)</option>
                            <option value="out" {{ $transaction->type === 'out' ? 'selected' : '' }}>إخراج (صرف من المخزون)</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity"
                           min="1" value="{{ old('quantity', $transaction->quantity) }}"
                           class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                           required>
                    @error('quantity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
