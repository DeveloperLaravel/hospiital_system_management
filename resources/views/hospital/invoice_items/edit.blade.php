...
<div class="p-6 bg-gray-50 min-h-screen" dir="rtl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('invoices.items.update', [$invoice, $item]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="service" value="{{ old('service', $item->service) }}" 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                              placeholder="تفاصيل إضافية اختيارية...">{{ old('description', $item->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
