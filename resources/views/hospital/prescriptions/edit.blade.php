    ...
    <div class="p-6 bg-gray-50 min-h-screen" dir="rtl">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold">تعديل وصفة طبية</h1>
                <a href="{{ route('prescriptions.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                    رجوع
                </a>
            </div>

            <form action="{{ route('prescriptions.update', $prescription) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="notes" class="w-full border rounded px-3 py-2" rows="3" placeholder="ملاحظات إضافية">{{ $prescription->notes }}</textarea>
                </div>
                    ...
                    <div id="items-container" class="space-y-4">
                        ...
                        <div class="item-row grid grid-cols-1 md:grid-cols-6 gap-2 p-4 bg-gray-50 rounded">
                            <div class="md:col-span-2">
                                ...
                                <select name="items[{{ $index }}][medication_id]" class="w-full border rounded px-2 py-1" required>
                            ...
                            <div>
                                ...
                                <input type="text" name="items[{{ $index }}][dosage]" value="{{ $item->dosage }}" class="w-full border rounded px-2 py-1" required>
                            </div>
                            <div>
                                <label class="block text-xs mb-1">التكرار</label>
                                <input type="text" name="items[{{ $index }}][frequency]" value="{{ $item->frequency }}" class="w-full border rounded px-2 py-1" required>
                            </div>
                            <div>
                                <label class="block text-xs mb-1">المدة</label>
                                <input type="text" name="items[{{ $index }}][duration]" value="{{ $item->duration }}" class="w-full border rounded px-2 py-1" required>
                            </div>
                            <div>
                                <label class="block text-xs mb-1">الكمية</label>
                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" class="w-full border rounded px-2 py-1" min="1" required>
                            </div>
                        </div>
                        @endforeach
                    </div>
